...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const { cameras, status } = req.body;

      if (!uid) {
        return res.status(400).json({
          success: false,
          message: "Operator UID is required",
        });
      }
      ...
      if (cameras !== undefined) {
        if (!Array.isArray(cameras) || cameras.length === 0) {
          return res.status(400).json({
            success: false,
            message: "At least one camera must be assigned",
          });
        }
      ...
      return res.status(500).json({
        success: false,
        message: err.message || "Failed to update operator",
      });
    }
  }
);
